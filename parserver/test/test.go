// test.go is a general purpose program that will run automated tests and generate scores
...
func runTests(rubric Rubric, GSPtr *bool, GSVisPtr *string) {
	...
	var spawnTest bool
	var gs GS
	gs.Tests = make([]GSTest, len(rubric.Items))
	...
	if total != float32(rubric.Total) {
		panic("Error: total score is not equal to total points.")
	}
	...
	if !(*GSPtr) {
		...
		gs.Score = pScore
		gs.Visibility = *GSVisPtr
		gs.StdOutVis = *GSVisPtr
...
func main() {
	...
	var assignId string
	var testFile string
	GSPtr := flag.Bool("gs", false, "Output should be in a format for gs")
	GSVisPtr := flag.String("gs-visibility", "after_published", "Provide the test visibility for gs")

	flag.Usage = func() {
		fmt.Fprintf(os.Stderr, "Usage: test ASSIGNMENT_ID\n")
	...
	if len(flag.Args()) != 1 {
		...
		runTests(rubric, GSPtr, GSVisPtr)
	}
}
