...
func runAllRequests(threads, version string, postInfo []int) {
	...
	if version == "p" {
		cmd = exec.CommandContext(ctx, "go", "run", "parserver/twitter", threads)
	} else {
		cmd = exec.CommandContext(ctx, "go", "run", "parserver/twitter")
	}
	stdout, err := cmd.StdoutPipe()
	if err != nil {
		fmt.Errorf("<runTwitter>: error in getting stdout pipe")
		os.Exit(1)
	}
	stdin, errIn := cmd.StdinPipe()
	if errIn != nil {
		fmt.Errorf("<runTwitter>: error in getting stdin pipe")
		os.Exit(1)
	}

	if err := cmd.Start(); err != nil {
		fmt.Errorf("<cmd.Start> error in executing Test")
		os.Exit(1)
	}
	...
	go func() {
		...
		for idx := 0; idx < len(postInfo); idx++ {
			...
			if err := encoder.Encode(&requestAdd); err != nil {
				fmt.Errorf("<AllRequests> add cmd.encode error in executing Test")
				os.Exit(1)
			}
		}
		for idx := addIdx; idx < (addIdx + len(postInfo)); idx++ {
			...
			if err := encoder.Encode(&requestContains); err != nil {
				fmt.Errorf("<AllRequests> request cmd.encode error in executing Test")
				os.Exit(1)
			}
		}
		<-wave1Done
		for idx := containsIdx; idx < (containsIdx + len(postInfo)/2); idx++ {
			...
			if err := encoder.Encode(&requestRemove); err != nil {
				fmt.Errorf("<AllRequests> remove cmd.encode error in executing Test")
				os.Exit(1)
			}
		}
		<-wave2Done
		...
		for idx := removeIdx; idx < (removeIdx + len(postInfo)/2); idx++ {
			...
			if err := encoder.Encode(&requestContains); err != nil {
				fmt.Errorf("<AllRequests> contains cmd.encode error in executing Test")
				os.Exit(1)
			}
		}
		...
		if err := encoder.Encode(&doneRequest); err != nil {
			fmt.Errorf("<AllRequests> done cmd.encode error in executing Test")
			os.Exit(1)
		}
		inDone <- true
	}()
